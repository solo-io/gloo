				...
				It("should add an sds sidecar AND an istio-proxy sidecar in the Gateway-Proxy Deployment", func() {
					...
					testManifest.SelectResources(func(resource *unstructured.Unstructured) bool {
						...
						if structuredDeployment.GetName() == "gateway-proxy" {
							...
							Ω(istioSidecarVersion(structuredDeployment.Spec.Template.Spec.Containers)).To(Equal("docker.io/istio/proxyv2:1.15.7"), "istio proxy sidecar should be the default")
...
spec:
  ...
  - second-gateway-proxy`
				...
				Context("gateway-proxy deployment", func() {
					...
					It("can overwrite sds and istioProxy images from Istio SDS", func() {
						prepareMakefile(namespace, helmValues{
							valuesArgs: []string{
								"global.glooMtls.enabled=true",
								"global.istioSDS.enabled=true",
...
`)
			...
			Describe("Standard k8s values", func() {
				DescribeTable("PodSpec affinity, tolerations, nodeName, hostAliases, nodeSelector, restartPolicy, initContainers, on Deployments and Jobs",
