...
func Bootstrap(opts bootstrap.Options) (secretwatcher.Interface, error) {
	...
	case bootstrap.WatcherTypeFile:
		client, err = file.NewSecretStorage(opts.FileOptions.SecretDir, opts.SecretStorageOptions.SyncFrequency)
		if err != nil {
	...
	case bootstrap.WatcherTypeKube:
		...
		client, err = kube.NewSecretStorage(cfg, opts.KubeOptions.Namespace, opts.SecretStorageOptions.SyncFrequency)
		if err != nil {
	...
	case bootstrap.WatcherTypeVault:
		...
		client = vault.NewSecretStorage(vaultClient, opts.VaultOptions.RootPath, opts.SecretStorageOptions.SyncFrequency)
	default:
