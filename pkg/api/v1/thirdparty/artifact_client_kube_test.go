...
var _ = Describe("ThirdPartyResourceClient", func() {
	...
	var (
		namespace string
		cfg       *rest.Config
		client    ThirdPartyResourceClient
	)
	BeforeEach(func() {
		...
		Expect(err).NotTo(HaveOccurred())
		resourceClient, err := versioned.NewForConfig(cfg, ArtifactCrd)
		Expect(err).NotTo(HaveOccurred())
		...
		client = NewArtifactClient(clientFactory)
	})
	AfterEach(func() {
