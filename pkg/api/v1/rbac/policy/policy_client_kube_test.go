...
var _ = Describe("PolicyClient", func() {
	...
	BeforeEach(func() {
		...
		Expect(err).NotTo(HaveOccurred())
		resourceClient, err := versioned.NewForConfig(cfg, PolicyCrd)
		Expect(err).NotTo(HaveOccurred())
		...
		client = NewPolicyClient(clientFactory)
	})
	AfterEach(func() {
	...
	It("CRUDs resources", func() {
		...
		Expect(r1.Resources).To(Equal(input.Resources))
		Expect(r1.Capabilities).To(Equal(input.Capabilities))
