...
var _ = Describe("IdentityClient", func() {
	...
	BeforeEach(func() {
		...
		Expect(err).NotTo(HaveOccurred())
		resourceClient, err := versioned.NewForConfig(cfg, IdentityCrd)
		Expect(err).NotTo(HaveOccurred())
		...
		client = NewIdentityClient(clientFactory)
	})
	AfterEach(func() {
	...
	It("CRUDs resources", func() {
		...
		name := "foo"
		input := NewIdentity(namespace, name)
