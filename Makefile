...
ENVOY_GLOO_IMAGE ?= gcr.io/gloo-ee/envoy-gloo-ee:1.20.4-patch1
ENVOY_GLOO_FIPS_IMAGE ?= gcr.io/gloo-ee/envoy-gloo-ee-fips:1.20.4-patch1
...
update-all-deps: install-go-tools
...
run-ci-gloo-fed-regression-tests: update-all-deps
	go env -w GOPRIVATE=github.com/solo-io
	RUNNING_REGRESSION_TESTS=true REMOTE_CLUSTER_CONTEXT=kind-remote LOCAL_CLUSTER_CONTEXT=kind-local $(DEPSGOBIN)/ginkgo -r ./test/gloo-fed-e2e/...
