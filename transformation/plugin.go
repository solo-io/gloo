...
func parsePathParameterToRegex(path string) ([]string, string) {
	...
	return parameterNames, rxp.ReplaceAllString(path, "([_[:alnum:]]+)")
}
...
func (p *Plugin) ProcessRoute(_ *plugin.RoutePluginParams, in *v1.Route, out *envoyroute.Route) error {
	...
	if spec.Input.PathParameter != "" {
		paramNames, pathRegexMatcher := parsePathParameterToRegex(spec.Input.PathParameter)
