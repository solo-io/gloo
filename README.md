...
The following programs must be installed in order to follow the subsequent guidelines:
 ...
 - Golang - install the version specified in [go.mod](./go.mod)
...
```bash
TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' make docker
```
...
The following variables can be set to affect the output:
 ...
 |`IMAGE_REGISTRY`|quay.io/solo-io|Docker image repository that built images are tagged for. Just put your usernameto refer to your account at hub.docker.com. To use gcr images, set the `IMAGE_REGISTRY` to `gcr.io/<PROJECT_NAME>`|
...
### Notes:
 - You will need the gcr.io/gloo-ee/envoy-gloo-ee Docker image in order to build the gloo-ee and gloo-ee-envoy-wrapper images
   ...
   - Ex: `TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' ENVOY_GLOO_IMAGE=gcr.io/gloo-ee/envoy-gloo-ee:1.19.0-patch4 make docker`
   - Because envoy-gloo-ee CI builds and publishes assets on PRs, you can also use the `ENVOY_GLOO_IMAGE` variable to point to a specific envoy-gloo-ee commit SHA
     - Ex: `TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' ENVOY_GLOO_IMAGE=gcr.io/gloo-ee/envoy-gloo-ee:<SHA> make docker`
     - Note that this will only work if envoy-gloo-ee CI has successfully built and published assets for the given SHA
 - You will need the gcr.io/gloo-ee/envoy-gloo-ee-fips Docker image in order to build the gloo-ee-fips and gloo-ee-envoy-wrapper-fips images
   ...
   - Ex: `TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' ENVOY_GLOO_FIPS_IMAGE=gcr.io/gloo-ee/envoy-gloo-ee-fips:1.19.0-patch4 make docker`
...
You can use the following command to push all Gloo Edge Enterprise Docker images to the repository specified by `IMAGE_REGISTRY`:
```bash
TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' make docker-push
```
 - Please note that the above command will have no effect if `TAGGED_VERSION` is not explicitly set
 - You should run the prior `make docker` step with the same values of `TAGGED_VERSION` and `IMAGE_REGISTRY` set in order to ensure that the proper images are pushed
 - You can push an individual image using the following command:
    ```bash
    docker push $IMAGE_REGISTRY/<IMAGE NAME>:$VERSION
    ```
...
```bash
CLUSTER_NAME=<CLUSTER NAME> TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' make push-kind-images
```

Where `CLUSTER_NAME` is set to the name of the Kind cluster, and `TAGGED_VERSION` and `IMAGE_REGISTRY` are set to the values used during the prior `make docker` step.

To load Gloo Fed images to kind use the `make gloofed-load-kind-images` target:
```bash
CLUSTER_NAME=<CLUSTER NAME> TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' make gloofed-load-kind-images
```
...
```bash
TAGGED_VERSION=<VERSION> IMAGE_REGISTRY='quay.io/solo-io' make build-test-chart
```

Where `TAGGED_VERSION` and `IMAGE_REGISTRY` are set to the values used during the prior `make docker` and `make docker-push` steps.
