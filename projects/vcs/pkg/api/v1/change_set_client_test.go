...
var _ = Describe("ChangeSetClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: ChangeSetCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewChangeSetClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs ChangeSets "+test.Description(), func() {
				ChangeSetClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ChangeSetClientTest(namespace string, client ChangeSetClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewChangeSet(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&ChangeSet{}))
	...
	Expect(r1.RootCommit).To(Equal(input.RootCommit))
