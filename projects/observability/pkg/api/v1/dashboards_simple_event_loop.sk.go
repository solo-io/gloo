...
// Deprecated: use DashboardsSyncDeciderWithContext
type DashboardsSyncDecider interface {
	DashboardsSyncer
	ShouldSync(old, new *DashboardsSnapshot) bool
}

type DashboardsSyncDeciderWithContext interface {
	DashboardsSyncer
	ShouldSync(ctx context.Context, old, new *DashboardsSnapshot) bool
}

type dashboardsSimpleEventLoop struct {
	emitter DashboardsSimpleEmitter
	syncers []DashboardsSyncer
}

func NewDashboardsSimpleEventLoop(emitter DashboardsSimpleEmitter, syncers ...DashboardsSyncer) eventloop.SimpleEventLoop {
	return &dashboardsSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *dashboardsSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[DashboardsSyncer]context.CancelFunc)
		...
		var previousSnapshot *DashboardsSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(DashboardsSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(DashboardsSyncDeciderWithContext); isDecider {
					...
					startTime := time.Now()
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("dashboards.observability.solo.io.SimpleEventLoopSync-%T", syncer))
