...
var _ = Describe("ResolverMapClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs ResolverMaps "+test.Description(), func() {
				ResolverMapClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ResolverMapClientTest(namespace string, client ResolverMapClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Eventually(func() ResolverMapList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
