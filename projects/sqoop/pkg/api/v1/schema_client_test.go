...
var _ = Describe("SchemaClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: SchemaCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewSchemaClient(factory.NewResourceClientFactory(factoryOpts))
...
func SchemaClientTest(namespace string, client SchemaClient) {
	...
	Expect(r1).To(BeAssignableToTypeOf(&Schema{}))
