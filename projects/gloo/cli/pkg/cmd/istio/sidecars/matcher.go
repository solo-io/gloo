...
func GetIstioSidecar(istioVersion, jwtPolicy string, istioMetaMeshID string, istioMetaClusterID string) (*corev1.Container, error) {
	if strings.HasPrefix(istioVersion, "1.7.") || strings.HasPrefix(istioVersion, "1.8.") || strings.HasPrefix(istioVersion, "1.9.") {
		return generateIstio17or18or19Sidecar(istioVersion, jwtPolicy, istioMetaMeshID, istioMetaClusterID), nil
