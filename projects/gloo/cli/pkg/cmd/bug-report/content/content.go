...
import (
	"fmt"
	client_go "github.com/solo-io/gloo/projects/gloo/cli/pkg/cmd/bug-report/client-go"
	"strings"
)
...
func (p *Params) SetClient(client client_go.Client) *Params {
	out := *p
	out.Client = client
	return &out
}
...
func (p *Params) SetGlooNamespace(namespace string) *Params {
	out := *p
	out.GlooNamespace = namespace
	return &out
}
...
func GetK8sResources(p *Params) (map[string]string, error) {
	out, err := client_go.RunCmd("get --all-namespaces "+
...
func GetSecrets(p *Params) (map[string]string, error) {
	...
	out, err := client_go.RunCmd(cmdStr, "", p.KubeConfig, p.KubeContext, p.DryRun)
...
func GetCRs(p *Params) (map[string]string, error) {
	...
	out, err := client_go.RunCmd("get --all-namespaces "+strings.Join(crds, ",")+" -o yaml", "", p.KubeConfig, p.KubeContext, p.DryRun)
	return retMap("crs", out, err)
}

// GetClusterInfo returns the cluster info.
func GetClusterInfo(p *Params) (map[string]string, error) {
	out, err := client_go.RunCmd("config current-context", "", p.KubeConfig, p.KubeContext, p.DryRun)
	if err != nil {
		return nil, err
	}
	...
	out, err = client_go.RunCmd("version", "", p.KubeConfig, p.KubeContext, p.DryRun)
	if err != nil {
		return nil, err
	}
...
func GetClusterContext(kubeConfig string) (string, error) {
	return client_go.RunCmd("config current-context", "", kubeConfig, "", false)
}

// GetNodeInfo returns node information.
func GetNodeInfo(p *Params) (map[string]string, error) {
	out, err := client_go.RunCmd("describe nodes", "", p.KubeConfig, p.KubeContext, p.DryRun)
...
func GetDescribePods(p *Params) (map[string]string, error) {
	if p.GlooNamespace == "" {
		return nil, fmt.Errorf("getDescribePods requires the Gloo namespace")
	}
	out, err := client_go.RunCmd("describe pods", p.GlooNamespace, p.KubeConfig, p.KubeContext, p.DryRun)
...
func GetEvents(p *Params) (map[string]string, error) {
	out, err := client_go.RunCmd("get events --all-namespaces -o wide", "", p.KubeConfig, p.KubeContext, p.DryRun)
...
func GetProxyInfo(p *Params) (map[string]string, error) {
	if p.Namespace == "" {
		return nil, fmt.Errorf("getProxyInfo requires namespace")
	}
	ret := make(map[string]string)
	for _, url := range proxyDebugURLs {
		out, err := client_go.EnvoyGet(p.Client, p.Namespace, p.Pod, url, p.DryRun)
		if err != nil {
			return nil, err
		}
		ret[url] = out
...
//		resource.Namespace(p.Namespace), resource.Namespace(p.GlooNamespace), nil, true, timeout)
//
...
/*func GetNetfilter(p *Params) (map[string]string, error) {
	...
	out, err := utils.RunCmd("exec -it -n "+p.Namespace+" "+p.Pod+
...
//	out, err := utils.Exec(p.Client, p.Namespace, p.Pod, p.Container, fmt.Sprintf("find %s -name core.*", coredumpDir), p.DryRun)
//	if err != nil {
//		return nil, err
//	}
//	var cds []string
...
func getCRDList(p *Params) ([]string, error) {
	crdStr, err := client_go.RunCmd("get customresourcedefinitions --no-headers", "", p.KubeConfig, p.KubeContext, p.DryRun)
	if err != nil {
		return nil, err
	}
	var out []string
