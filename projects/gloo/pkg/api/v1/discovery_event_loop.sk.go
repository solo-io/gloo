...
var (
	mDiscoverySnapshotTimeSec     = stats.Float64("discovery.gloo.solo.io/sync/time_sec", "The time taken for a given sync", "1")
	mDiscoverySnapshotTimeSecView = &view.View{
		Name:        "discovery.gloo.solo.io/sync/time_sec",
		Description: "The time taken for a given sync",
		TagKeys:     []tag.Key{tag.MustNewKey("syncer_name")},
		Measure:     mDiscoverySnapshotTimeSec,
	...
	go func() {
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				if syncDecider, isDecider := el.syncer.(DiscoverySyncDecider); isDecider {
					if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
						continue // skip syncing this syncer
					}
				} else if syncDeciderWithContext, isDecider := el.syncer.(DiscoverySyncDeciderWithContext); isDecider {
				...
				stats.RecordWithTags(
					...
					mDiscoverySnapshotTimeSec.M(time.Now().Sub(startTime).Seconds()),
				)
				span.End()
