...
var _ = Describe("EndpointClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs Endpoints "+test.Description(), func() {
				EndpointClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func EndpointClientTest(namespace string, client EndpointClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
