...
// Equal function
func (m *Settings_DiscoveryOptions_GraphqlDiscoveryOptions) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*Settings_DiscoveryOptions_GraphqlDiscoveryOptions)
	if !ok {
		that2, ok := that.(Settings_DiscoveryOptions_GraphqlDiscoveryOptions)
		if ok {
			target = &that2
		} else {
			return false
