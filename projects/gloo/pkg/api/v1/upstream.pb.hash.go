...
func (m *Upstream) Hash(hasher hash.Hash64) (uint64, error) {
	...
	if h, ok := interface{}(m.GetCoolNewField()).(safe_hasher.SafeHasher); ok {
		if _, err = hasher.Write([]byte("CoolNewField")); err != nil {
			return 0, err
		}
		if _, err = h.Hash(hasher); err != nil {
			return 0, err
		...
		if fieldValue, err := hashstructure.Hash(m.GetCoolNewField(), nil); err != nil {
			return 0, err
		} else {
			if _, err = hasher.Write([]byte("CoolNewField")); err != nil {
				return 0, err
	...
	if _, err = hasher.Write([]byte(m.GetNonNullableNewField())); err != nil {
		return 0, err
	}
