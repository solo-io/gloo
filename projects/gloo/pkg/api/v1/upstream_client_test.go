...
var _ = Describe("UpstreamClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs Upstreams "+test.Description(), func() {
				UpstreamClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func UpstreamClientTest(namespace string, client UpstreamClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
