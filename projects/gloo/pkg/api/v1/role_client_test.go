...
var _ = Describe("RoleClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: RoleCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewRoleClient(factory.NewResourceClientFactory(factoryOpts))
			...
			It("CRUDs Roles", func() {
				RoleClientTest(namespace, client)
			})
		})
	}
})

func RoleClientTest(namespace string, client RoleClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
