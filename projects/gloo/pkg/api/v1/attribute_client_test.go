...
var _ = Describe("AttributeClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: AttributeCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewAttributeClient(factory.NewResourceClientFactory(factoryOpts))
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs Attributes", func() {
				AttributeClientTest(namespace, client)
			})
		})
	}
})

func AttributeClientTest(namespace string, client AttributeClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := "foo"
	input := NewAttribute(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&Attribute{}))
