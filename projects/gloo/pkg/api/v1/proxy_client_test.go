...
var _ = Describe("ProxyClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs Proxys "+test.Description(), func() {
				ProxyClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ProxyClientTest(namespace string, client ProxyClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
