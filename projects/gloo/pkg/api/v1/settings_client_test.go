...
var _ = Describe("SettingsClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs Settingss", func() {
				SettingsClientTest(namespace, client)
			})
		})
	}
})

func SettingsClientTest(namespace string, client SettingsClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Expect(r1.KubernetesConfigSource).To(Equal(input.KubernetesConfigSource))
	Expect(r1.DirectoryConfigSource).To(Equal(input.DirectoryConfigSource))
	Expect(r1.KubernetesSecretSource).To(Equal(input.KubernetesSecretSource))
	Expect(r1.VaultSecretSource).To(Equal(input.VaultSecretSource))
	Expect(r1.DirectorySecretSource).To(Equal(input.DirectorySecretSource))
	Expect(r1.KubernetesArtifactSource).To(Equal(input.KubernetesArtifactSource))
	Expect(r1.DirectoryArtifactSource).To(Equal(input.DirectoryArtifactSource))
