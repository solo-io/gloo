...
// of the field name in the hash calculation can lead to hash collisions.
// Prefer the HashUnique function instead.
func (m *RateLimitedRetryBackOff) Hash(hasher hash.Hash64) (uint64, error) {
	if m == nil {
		return 0, nil
	}
	if hasher == nil {
	...
	if h, ok := interface{}(m.GetIncludeResetHeaders()).(safe_hasher.SafeHasher); ok {
		if _, err = hasher.Write([]byte("IncludeResetHeaders")); err != nil {
			return 0, err
		}
		if _, err = h.Hash(hasher); err != nil {
			return 0, err
		...
		if fieldValue, err := hashstructure.Hash(m.GetIncludeResetHeaders(), nil); err != nil {
			return 0, err
		} else {
			if _, err = hasher.Write([]byte("IncludeResetHeaders")); err != nil {
				return 0, err
