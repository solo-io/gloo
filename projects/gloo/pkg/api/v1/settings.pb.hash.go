...
func (m *GatewayOptions_ValidationOptions) Hash(hasher hash.Hash64) (uint64, error) {
	...
	if h, ok := interface{}(m.GetEnableExtensionValidation()).(safe_hasher.SafeHasher); ok {
		if _, err = hasher.Write([]byte("EnableExtensionValidation")); err != nil {
			return 0, err
		}
		if _, err = h.Hash(hasher); err != nil {
			return 0, err
		...
		if fieldValue, err := hashstructure.Hash(m.GetEnableExtensionValidation(), nil); err != nil {
			return 0, err
		} else {
			if _, err = hasher.Write([]byte("EnableExtensionValidation")); err != nil {
				return 0, err
