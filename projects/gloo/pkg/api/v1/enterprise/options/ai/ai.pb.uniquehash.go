...
func (m *UpstreamSpec_BedRock) HashUnique(hasher hash.Hash64) (uint64, error) {
	...
	case *UpstreamSpec_BedRock_KeySecretRef:
		...
		if h, ok := interface{}(m.GetKeySecretRef()).(safe_hasher.SafeHasher); ok {
			if _, err = hasher.Write([]byte("KeySecretRef")); err != nil {
				return 0, err
			}
			if _, err = h.Hash(hasher); err != nil {
				return 0, err
			...
			if fieldValue, err := hashstructure.Hash(m.GetKeySecretRef(), nil); err != nil {
				return 0, err
			} else {
				if _, err = hasher.Write([]byte("KeySecretRef")); err != nil {
					return 0, err
