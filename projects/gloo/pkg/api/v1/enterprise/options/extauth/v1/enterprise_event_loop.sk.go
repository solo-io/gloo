	...
	go func() {
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				if syncDecider, isDecider := el.syncer.(EnterpriseSyncDecider); isDecider {
					if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
						continue // skip syncing this syncer
					}
				} else if syncDeciderWithContext, isDecider := el.syncer.(EnterpriseSyncDeciderWithContext); isDecider {
