...
var _ = Describe("UpstreamGroupClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: UpstreamGroupCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewUpstreamGroupClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs UpstreamGroups "+test.Description(), func() {
				UpstreamGroupClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func UpstreamGroupClientTest(namespace string, client UpstreamGroupClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewUpstreamGroup(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&UpstreamGroup{}))
	...
	Eventually(func() UpstreamGroupList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
