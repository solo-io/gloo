...
func (m *OpenTelemetryConfig) Hash(hasher hash.Hash64) (uint64, error) {
	...
	if h, ok := interface{}(m.GetMaxCacheSize()).(safe_hasher.SafeHasher); ok {
		if _, err = hasher.Write([]byte("MaxCacheSize")); err != nil {
			return 0, err
		}
		...
		if fieldValue, err := hashstructure.Hash(m.GetMaxCacheSize(), nil); err != nil {
			return 0, err
		} else {
			if _, err = hasher.Write([]byte("MaxCacheSize")); err != nil {
				return 0, err
