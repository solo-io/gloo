...
func (m *ServerNameMatcher) HashUnique(hasher hash.Hash64) (uint64, error) {
	if m == nil {
		return 0, nil
	}
	if hasher == nil {
	...
	if _, err = hasher.Write([]byte("ServerNameMatchers")); err != nil {
		return 0, err
	}
	for i, v := range m.GetServerNameMatchers() {
...
// Replaces Hash due to original hashing implemention only using field values. The omission
// of the field name in the hash calculation can lead to hash collisions.
func (m *ServerNameMatcher_ServerNameSetMatcher) HashUnique(hasher hash.Hash64) (uint64, error) {
	if m == nil {
		return 0, nil
	}
	if hasher == nil {
	...
	if _, err = hasher.Write([]byte("ServerNames")); err != nil {
		return 0, err
	}
	for i, v := range m.GetServerNames() {
		if _, err = hasher.Write([]byte(strconv.Itoa(i))); err != nil {
			return 0, err
		}
