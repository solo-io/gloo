...
type ConfigCollector interface {
	...
	ToXdsConfiguration() *enterprise.RateLimitConfig
}

type ConfigCollectorFactory interface {
	MakeInstance(typ CollectorType, snapshot *v1.ApiSnapshot, reports reporter.ResourceReports) (ConfigCollector, error)
}
