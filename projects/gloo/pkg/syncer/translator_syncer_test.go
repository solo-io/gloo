...
var _ = Describe("Translate Proxy", func() {
	...
	BeforeEach(func() {
		...
		syncer = NewTranslatorSyncer(ctx, &mockTranslator{true, false, nil}, xdsCache, sanitizer, rep, false, nil, settings, statusMetrics, nil, proxyClient, "", singlereplica.Identity(), nil, iosnapshot.NewHistory(xdsCache, settings))
		snap = &v1snap.ApiSnapshot{
		...
		syncer = NewTranslatorSyncer(ctx, &mockTranslator{false, false, nil}, xdsCache, sanitizer, rep, false, nil, settings, statusMetrics, nil, proxyClient, "", singlereplica.Identity(), nil, iosnapshot.NewHistory(xdsCache, settings))
...
var _ = Describe("Translate multiple proxies with errors", func() {
	...
	BeforeEach(func() {
		...
		syncer = NewTranslatorSyncer(ctx, &mockTranslator{true, true, nil}, xdsCache, sanitizer, rep, false, nil, settings, statusMetrics, nil, proxyClient, "", singlereplica.Identity(), nil, iosnapshot.NewHistory(xdsCache, settings))
		snap = &v1snap.ApiSnapshot{
			Proxies: v1.ProxyList{
				proxy1,
				proxy2,
