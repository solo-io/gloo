...
var _ = Describe("Plugin", func() {
	Context("Route with HTTP Proxy Upstream", func() {
		...
		BeforeEach(func() {
			params = plugins.Params{
				Snapshot: &v1snap.ApiSnapshot{
					Upstreams: []*v1.Upstream{routeReferencedUs},
				},
			}
									...
									Action: &envoy_config_route_v3.Route_Route{
										Route: &envoy_config_route_v3.RouteAction{
											ClusterSpecifier: &envoy_config_route_v3.RouteAction_Cluster{
												Cluster: translator.UpstreamToClusterName(routeReferencedUs.Metadata.Ref()),
											},
										},
									},
								},
			...
			clusterName := translator.UpstreamToClusterName(routeReferencedUs.Metadata.Ref())
			inClusters = []*envoy_config_cluster_v3.Cluster{
				{
					Name: clusterName,
		...
		It("should update resources properly", func() {
			...
			adjustedOriginalCluster := originalCluster + "_original"
			...
			Expect(typedTcpConfig.GetCluster()).To(Equal(adjustedOriginalCluster), "should forward to original destination")
		})
		...
		Context("multiple routes and clusters", func() {
			...
			BeforeEach(func() {
				...
				usCopy := &v1.Upstream{}
				routeReferencedUs.DeepCopyInto(usCopy)
	...
	// Context("Non-route upstream", func() {
	// 	var (
	// 		params                plugins.Params
	// 		inClusters            []*envoy_config_cluster_v3.Cluster
	// 		inRouteConfigurations []*envoy_config_route_v3.RouteConfiguration
	...
	// 			},
	// 			SslConfig:         nil,
	// 			HttpProxyHostname: &wrappers.StringValue{Value: "jwks.com:443"},
	// 		}
	// 	)
