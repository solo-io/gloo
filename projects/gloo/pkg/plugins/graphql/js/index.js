...
if (!protoImportPath) {
  console.error("stitching tools script requires STITCHING_PROTO_DIR environment variable")
  process.exit(1)
}

const {GraphQLToolsStitchingInput, GraphQlToolsStitchingOutput } = require(protoImportPath + "/v1/enterprise/options/graphql/v1alpha1/stitching_info_pb");
const {MergedTypeConfig, FieldNodeMap, FieldNodes, FieldNode, Schemas} = require(protoImportPath+ "/external/envoy/extensions/graphql/stitching_pb");


async function makeStitchedSchema(input) {
  const inMsg = base64StringToMsg(input, GraphQLToolsStitchingInput.deserializeBinary);
  let subschemas = [];
  ...
  for (let i = 0; i < subschemaList.length; i++) {
    ...
    if (!subschema.getSchema()) {
      console.error(`subschema '${subschema.getName()}' is missing a schema definition`);
      process.exit(1);
    }
    subschemaConfig.schema = makeExecutableSchema({typeDefs: subschema.getSchema()});
...
function convertStitchingInfo(si) {
  ...
  for (let fieldNodeByFieldIdx in si.fieldNodesByField) {
    ...
    for (let fieldNodeType in si.fieldNodesByField[fieldNodeByFieldIdx]){
      ...
      for (let i = 0; i <  si.fieldNodesByField[fieldNodeByFieldIdx][fieldNodeType].length; i++) {
        let newFieldNode = new FieldNode();
        newFieldNode.setName(si.fieldNodesByField[fieldNodeByFieldIdx][fieldNodeType][i].name.value);
  ...
  for (let type in si.mergedTypes) {
    ...
    for (let [subschema, selectionSet] of mt.selectionSets) {
      newMtConfig.getSelectionSetsMap().set(subschemaMap.get(subschema), graphql.print(selectionSet));
    }
    for (let [subschema, targetSubschemas] of mt.targetSubschemas) {
      ...
      newMtConfig.getDeclarativeTargetSubschemasMap().set(subschemaMap.get(subschema), newTargetSubschemas);
    }

    for (let [fieldName, subschema] of Object.entries(mt.uniqueFields)) {
      newMtConfig.getUniqueFieldsToSubschemaNameMap().set(fieldName, subschemaMap.get(subschema));
    }
