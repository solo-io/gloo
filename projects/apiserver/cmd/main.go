...
func main() {
	...
	if err := fields.AddGlooInstanceIndexer(rootCtx, mgr); err != nil {
		contextutils.LoggerFrom(rootCtx).Fatalw("A fatal error occurred while adding cluster indexer to GlooInstance", zap.Error(err))
	}
	...
	if err := mgr.Add(apiserver.NewServerRunnable(rootCtx, cfg, glooInstanceService, failoverSchemeService, routeTableSelectorService,
		...
		contextutils.LoggerFrom(rootCtx).Fatalw("Unable to set up apiserver", zap.Error(err))
	}

	if err := clusterWatcher.Run(mgr); err != nil {
		contextutils.LoggerFrom(rootCtx).Fatalw("A fatal error occurred while starting the cluster watcher", zap.Error(err))
	}
