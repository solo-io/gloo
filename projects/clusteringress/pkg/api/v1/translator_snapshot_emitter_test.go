...
var _ = Describe("V1Emitter", func() {
	...
	It("tracks snapshots on changes to any resource", func() {
		...
		*/
		...
		clusterIngress1a, err := clusterIngressClient.Write(NewClusterIngress(namespace1, "angela"), clients.WriteOpts{Ctx: ctx})
		Expect(err).NotTo(HaveOccurred())
		clusterIngress1b, err := clusterIngressClient.Write(NewClusterIngress(namespace2, "angela"), clients.WriteOpts{Ctx: ctx})
		Expect(err).NotTo(HaveOccurred())

		assertSnapshotClusteringresses(ClusterIngressList{clusterIngress1a, clusterIngress1b}, nil)

		clusterIngress2a, err := clusterIngressClient.Write(NewClusterIngress(namespace1, "bob"), clients.WriteOpts{Ctx: ctx})
		Expect(err).NotTo(HaveOccurred())
		clusterIngress2b, err := clusterIngressClient.Write(NewClusterIngress(namespace2, "bob"), clients.WriteOpts{Ctx: ctx})
		Expect(err).NotTo(HaveOccurred())
