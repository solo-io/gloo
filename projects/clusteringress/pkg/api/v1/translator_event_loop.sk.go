	...
	go func() {
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				if syncDecider, isDecider := el.syncer.(TranslatorSyncDecider); isDecider {
					if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
						continue // skip syncing this syncer
					}
				} else if syncDeciderWithContext, isDecider := el.syncer.(TranslatorSyncDeciderWithContext); isDecider {
