...
/*
	...
	It("creates the appropriate proxy object for the provided ingress objects", func() {
		...
		for _, requireIngressClass := range []bool{true, false} {
			ingress := &extensions.Ingress{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "ing",
					Namespace: namespace,
			...
			cliutils.PrintYAML(proxy, os.Stderr)
			cliutils.PrintYAML(expected, os.Stderr)
			Expect(proxy.String()).To(Equal((expected).String()))
		}
	})
*/
