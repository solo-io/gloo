...
var _ = Describe("LicenseKeyClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: LicenseKeyCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewLicenseKeyClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs LicenseKeys "+test.Description(), func() {
				LicenseKeyClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func LicenseKeyClientTest(namespace string, client LicenseKeyClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewLicenseKey(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&LicenseKey{}))
	...
	Expect(r1.Key).To(Equal(input.Key))
	...
	Eventually(func() LicenseKeyList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
