...
import (
	...
	"github.com/solo-io/gloo/projects/controllerrollerroller/cli/pkg/cmd/options/contextoptions"
...
func knativeCmd(opts *options.Options) *cobra.Command {
	cmd := &cobra.Command{
		...
		Deprecated: "Knative with Gloo is deprecated in k8sgateway 1.10 and will not be available in k8sgateway 1.11",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !opts.Install.Knative.SkipGlooInstall {
				...
				if err := NewInstaller(opts, DefaultHelmClient()).Install(&InstallerConfig{
					...
					return eris.Wrapf(err, "installing k8sgateway in knative mode")
				}
			}
			return nil
		},
