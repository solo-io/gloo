...
var _ = Describe("Install", func() {
	...
	It("should error when not overriding helm chart in dev mode", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("installing k8sgateway in ingress mode: you must provide a Gloo Helm chart URI via the 'file' option when running an unreleased version of glooctl"))
	})
	...
	It("should error when not providing file with valid extension", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("installing k8sgateway in gateway mode: unsupported file extension for Helm chart URI: [foo]. Extension must either be .tgz or .tar.gz"))
	})
