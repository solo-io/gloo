...
- k8sgateway needs AWS credentials to be able to find EC2 resources
...
## Create roles for k8sgateway to assume on behalf of your upstreams
- For additional control over k8sgateway's access to your resources and as an additional filter on your EC2 Upstream's list of
...
- When you provide both a secret ref and a list of Role ARNs to your upstream, k8sgateway will call the AWS API with credentials
...
- Alternatively, the user could just provide credentials, and allow k8sgateway to discover the specs by inspection of the tags.
   ...
   "Statement":[
      {
         ...
         "Condition":{
            "StringEquals":{
               "ec2:ResourceTag/Owner":"k8sgateway"
            }
         }
      }
   ]
...
- The action that the EC2 upstream credentials must have is `ec2:DescribeInstances`.
  - [DescribeInstances](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeInstances.html) is the only AWS API that k8sgateway needs.
...
- The `ResourceTags`, in particular, are how k8sgateway chooses which EC2 instances to associate with a given upstream.
...
- k8sgateway uses the intersection of an upstream's credentials and its filter spec to determine which EC2 instances should be associated with an upstream.
