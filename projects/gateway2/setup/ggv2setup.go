...
func getInitialSecret(ctx context.Context, c istiokube.Client, nns types.NamespacedName) *corev1.Secret {
	// // get initial settings
	// logger := contextutils.LoggerFrom(ctx)
...
func StartGGv2WithConfig(ctx context.Context,
	...
	// DO_NOT_SUBMIT - Start secrets logic move to own function
	initialSecret := getInitialSecret(ctx, kubeClient, settingsNns)
	...
	if initialSecret != nil {
		...
		logger.Info("No initial secret found, using hardcoded certs")
	}
