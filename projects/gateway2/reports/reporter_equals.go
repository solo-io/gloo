...
// pointed-to content of the report maps, not pointer identity: every translation
// pass allocates a fresh ReportMap with fresh report pointers, so a pointer-based
// compare (maps.Equal / ==) always reports "changed" and keeps the control plane
// re-translating and re-emitting reports.
//
...
// Group pointers each pass, so a struct == / pointer compare would treat
// semantically identical kinds as different. The builder also ranges a map, so
// the slice order is not stable across passes.
...
// field that affects rendered status except LastTransitionTime.
