...
type GatewayLister interface {
	...
	List(selector labels.Selector) (ret []*v2.Gateway, err error)
...
// List lists all Gateways in the indexer.
func (s *gatewayLister) List(selector labels.Selector) (ret []*v2.Gateway, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.Gateway))
	})
	return ret, err
}
...
type GatewayNamespaceLister interface {
	...
	Get(name string) (*v2.Gateway, error)
...
// List lists all Gateways in the indexer for a given namespace.
func (s gatewayNamespaceLister) List(selector labels.Selector) (ret []*v2.Gateway, err error) {
...
// Get retrieves the Gateway from the indexer for a given namespace and name.
func (s gatewayNamespaceLister) Get(name string) (*v2.Gateway, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("gateway"), name)
	}
	return obj.(*v2.Gateway), nil
}
