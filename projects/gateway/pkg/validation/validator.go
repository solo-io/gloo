...
func (v *validator) validateSnapshotThreadSafe(ctx context.Context, resource resources.Resource, dryRun bool) (
	*Reports,
	...
	// TODO-JAKE ensure that the thread safe feature is enabled for all validations
	v.lock.Lock()
	defer v.lock.Unlock()
...
func (v *validator) validateSnapshot(ctx context.Context, resource resources.Resource, dryRun bool) (*Reports, error) {
	...
	// TODO-JAKE ensure that this is the only clone of the snapshot
	...
	for proxyName, gatewayList := range gatewaysByProxy {
		...
		// TODO-JAKE refactor this code so that it does what validateGlooResource() is currently doing
		// TODO-JAKE for the first run only have the check on the proxy Reports.  Then after testing of that, add in the resource reports as well.
...
func (v *validator) ValidateDeleteRef(ctx context.Context, gvk schema.GroupVersionKind, resource resources.Resource, dryRun bool) error {
	if gvk.Group == GatewayGroup {
		...
		if _, hit := gloovalidation.GvkToSupportedDeleteGlooResources[gvk]; hit {
			_, err := v.validateGlooResource(ctx, resource, true)
			return err
		}
	}
...
func (v *validator) ValidateGvk(ctx context.Context, gvk schema.GroupVersionKind, resource resources.Resource, dryRun bool) (*Reports, error) {
	...
	if gvk.Group == GatewayGroup {
		...
		if _, hit := gloovalidation.GvkToSupportedGlooResources[gvk]; hit {
			reports, err := v.validateGlooResource(ctx, resource, false)
			if err != nil {
				return reports, &multierror.Error{Errors: []error{errors.Wrapf(err, "Validating %T failed", resource)}}
			}
			return reports, nil
...
func (v *validator) copySnapshot() (*gloosnapshot.ApiSnapshot, error) {
	// TODO-JAKE may want to combine this with the copy for Gateway Resources
	...
	snapCopy := v.latestSnapshot.Clone() // cloning can mutate so we need a write lock
	return &snapCopy, nil
}
...
func (v *validator) validateGlooResource(ctx context.Context, resource resources.Resource, delete bool) (*Reports, error) {
	v.lock.Lock()
	defer v.lock.Unlock()
	snapshotCopy, err := v.copySnapshot()
	if err != nil {
		return &Reports{}, err
	}
	if resource != nil {
