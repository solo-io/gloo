...
// TestFullEnvoyValidation is the function which executes a series of tests against a given
// installation where full Envoy validation is enabled (fullEnvoyValidation=true)
func TestFullEnvoyValidation(t *testing.T) {
	ctx := context.Background()
	installNs, nsEnvPredefined := envutils.LookupOrDefault(testutils.InstallNamespace, "full-envoy-validation-test")
	...
	FullEnvoyValidationSuiteRunner().Run(ctx, t, testInstallation)
}
