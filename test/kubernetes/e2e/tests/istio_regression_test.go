...
// TestIstioRegression is the function which executes a series of tests against a given installation where
// the k8s Gateway controller is disabled and the deprecated Istio integration values are used to check for regressions
func TestIstioRegression(t *testing.T) {
	ctx := context.Background()
	installNs, nsEnvPredefined := envutils.LookupOrDefault(testutils.InstallNamespace, "istio-regression-test")
	testInstallation := e2e.CreateTestInstallation(
		t,
		&gloogateway.Context{
			InstallNamespace:   installNs,
			ValuesManifestFile: filepath.Join(util.MustGetThisDir(), "manifests", "istio-regression-helm.yaml"),
		},
	)
