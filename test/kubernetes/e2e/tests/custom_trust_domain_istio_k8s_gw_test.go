...
func TestK8sGatewayIstioCustomTrustDomain(t *testing.T) {
	...
	testInstallation := e2e.CreateTestInstallation(
		t,
		&gloogateway.Context{
			InstallNamespace:   "istio-trust-domain-k8s-gw-test",
			ValuesManifestFile: filepath.Join(util.MustGetThisDir(), "manifests", "istio-custom-trust-domain-k8s-gateway.yaml"),
		},
	)
	...
	CustomTrustDomainIstioK8sGatewaySuiteRunner().Run(ctx, t, testInstallation)
}
