...
*/
func NewGlooGatewayTestingSuite(ctx context.Context, testInst *e2e.TestInstallation) suite.TestingSuite {
...
func (s *glooGatewayPortRoutingTestingSuite) SetupSuite() {
	...
	err = s.testInstallation.Actions.Kubectl().ApplyFile(s.ctx, glooGatewayRoutingManifest, "-n", s.testInstallation.Metadata.InstallNamespace)
	s.NoError(err, "can apply glooGatewayRoutingManifest manifest")
}
...
func (s *glooGatewayPortRoutingTestingSuite) TearDownSuite() {
	...
	err = s.testInstallation.Actions.Kubectl().DeleteFile(s.ctx, glooGatewayRoutingManifest, "-n", s.testInstallation.Metadata.InstallNamespace)
	s.NoError(err, "can delete glooGatewayRoutingManifest manifest")
}
