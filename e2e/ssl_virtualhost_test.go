...
var _ = Describe("SNI Virtualhost", func() {
	...
	Context("creating a vhost with an ssl config", func() {
		...
		It("should get connection refused on port 8080 (http)", func() {
			curlEventuallyShouldRespond(curlOpts{path: path, port: 8080}, "Connection refused", time.Minute*5)
		})
		It("should get a 200ok on the ssl port (8443)", func() {
			curlEventuallyShouldRespond(curlOpts{protocol: "https", path: path, port: 8443, caFile: "/envoy.crt"}, "< HTTP/1.1 200", time.Minute*5)
		})
	})
})
