...
GLOO_FED=$(kubectl -n gloo-system rollout status deployment gloo-fed 2> /dev/null)
GLOO_FED_CONSOLE=$(kubectl -n gloo-system rollout status deployment gloo-fed-console 2> /dev/null)
if [[ "$GLOO_FED" = "deployment \"gloo-fed\" successfully rolled out" ]] && [[ "$GLOO_FED_CONSOLE" = "deployment \"gloo-fed-console\" successfully rolled out" ]]; then
  ...
  if [[ "HAS_CLUSTER_REGISTERED" == "" ]]; then
    ...
    echo "KIND: glooctl cluster register --cluster-name [kind-local] --local-cluster-domain-override host.docker.internal --remote-context [kind-local] --federation-namespace [default: gloo-system]"
    echo "GKE:  glooctl cluster register --cluster-name [gloo-fed-remote] --remote-context [gke-context-name] --federation-namespace [default: gloo-system]"
  fi
  # check port forwarding is already setup
  PORT_FWD=$( pgrep -f "kubectl port-forward -n gloo-system deploy/gloo-fed-console 8090")
  if [[ "$PORT_FWD" == "" ]]; then
    kubectl port-forward -n gloo-system deploy/gloo-fed-console 8090 &
  fi
  exit 0
else
