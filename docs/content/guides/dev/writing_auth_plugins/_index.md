...
weight: 7
description: Guidelines and best practices for developing and configuring Go plugins to extend Gloo Gateway's ext auth server
---

In the [**Plugin Auth** guide]({{% versioned_link_path fromRoot="/guides/security/auth/extauth/plugin_auth" %}}) 
we showed how easy it is to extend Gloo Gateway with custom authentication logic using Go plugins. That guide uses a 
...
- [Configuring Gloo Gateway to load your plugins](#configuring-gloo-edge-to-load-your-plugins)
...
version of Gloo Gateway Enterprise. Given the [constraints imposed by Go plugins](#build-helper-tools), these utilities will 
...
we gave a high-level description of the steps required to extend Gloo Gateway with your own plugins:

1. Write a plugin and publish it as a `docker image` which, when run, copies the compiled plugin file to a 
predefined directory.
2. Configure Gloo Gateway to load the plugin by running the image as an `initContainer` on the `extauth` deployment. This can be 
done by installing Gloo Gateway with [dedicated value overrides]({{% versioned_link_path fromRoot="/guides/security/auth/extauth/plugin_auth#installation" %}}) 
or by modifying the Gloo Gateway installation manifest manually.
...
In this section we will see how to develop an auth plugin and distribute it the format that Gloo Gateway expects.
...
##### ExtAuthPlugin
Gloo Gateway expects auth plugins to implement the 
...
Objects that implement this interface are used as factories for authentication service instances. After Gloo Gateway detects a 
...
Gloo Gateway will populate the struct fields with the values found on the correspondent YAML attributes. 
...
The `GetAuthService` function will be invoked by Gloo Gateway right after this step. As its `configInstance` argument, Gloo Gateway will 
...
##### AuthService
`AuthService` instances are responsible for authorizing individual requests. This is the interface that all of Gloo Gateway's 
out-of-the box auth implementations (basic auth, OIDC, etc.) implement as well. Your plugin is responsible for providing 
Gloo Gateway with a valid `AuthService` implementation.
...
The `Start` function will be called once by Gloo Gateway, when the auth service is started. It is intended as a hook to perform 
initialization logic or to start auxiliary processes that span the whole lifecycle of the service.

All the functions we have just described (`NewConfigInstance`, `GetAuthService`, and `Start`) will be invoked when Gloo Gateway 
detects a new auth configuration on your Virtual Services. The `Authorize` function, on the other hand, will be invoked 
each time a request hits Gloo Gateway and matches the virtual host on which the your plugin is defined. 
...
a few words on the lifecycle of `AuthService`s. You might have noticed that Gloo Gateway passes a `context.Context` to each of 
the functions we just saw. The context will live as long as the plugin configuration that generated it is valid. 
Whenever the auth configuration changes, Gloo Gateway will start new `AuthService` instances and signal the termination of the 
...
the following is the sequence of actions that the Gloo Gateway external auth service performs when it detects a change in an 
...
Earlier in this guide we mentioned that Gloo Gateway expects auth plugins to implement the 
...
In order for Gloo Gateway to be able to load your plugin, the `main` package of your plugin must export a variable that 
implements the [ExtAuthPlugin](https://github.com/solo-io/ext-auth-plugins/blob/master/api/interface.go#L62) interface. 
This is usually a struct or a pointer to a struct (Gloo Gateway is smart enough to handle both cases). 
Gloo Gateway will use the [Lookup function](https://golang.org/pkg/plugin/#Plugin.Lookup) to find the exported variable and 
assert that it in fact implements the expected interface.

You can specify the name of the variable Gloo Gateway looks for when you reference your plugin in your Virtual Services:
...
{{% notice note %}}
Gloo Gateway publishes information about the environment it was built with to a Google Storage bucket. The tools in this section 
will make use of this information. You can find the information for a specific Gloo Gateway version in the following files located 
under `http://storage.googleapis.com/gloo-ee-dependencies/[GLOOE_VERSION]`:

- `dependencies`: contains the versions of all dependencies used by Gloo Gateway (generated by running `go list -m all` on the Gloo Gateway Enterprise module)
- `build_env`: values that can be used to replicate the environment the given Gloo Gateway version was built in.
- `verify-plugins-linux-amd64`: a script to verify that the plugin can be loaded by the given Gloo Gateway version.

You can get all these files by running `GLOOE_VERSION=desired_version make get-glooe-info` in our example repository.
{{% /notice %}}

#### Compare dependencies
We manage Gloo Gateway dependencies using [Go Modules](https://github.com/golang/go/wiki/Modules). The `go.mod` file contains 
...
to take advantage of a script we provide for comparing the dependencies of your plugin with the Gloo Gateway ones. 
...
[
  {
    "message": "Please pin your dependency to the same version as the Gloo Gateway one using a [require] clause",
    "pluginDependencies": {
...
be able to use the information in the Gloo Gateway `dependencies` file to verify that the dependencies match.
...
As part of each Gloo Gateway Enterprise release, we ship a script to verify whether your plugin can be loaded by that version of 
Gloo Gateway Enterprise. You can find it in the aforementioned Google Cloud bucket at 
...
#### Dockerfile
We mentioned that the plugin must be compiled with the same `GOPATH` as Gloo Gateway. We also cannot easily cross-compile with 
...
{{< highlight yaml >}}
# This stage is parametrized to replicate the same environment Gloo Gateway Enterprise was built in.
# All ARGs need to be set via the docker `--build-arg` flags.
ARG GO_BUILD_IMAGE
FROM $GO_BUILD_IMAGE AS build-env

# This must contain the value of the `gcflag` build flag that Gloo Gateway was built with
ARG GC_FLAGS
...
# We need this so that the import paths for any library shared between the plugins and Gloo Gateway are the same.
#
...
# by the go runtime as two different types, causing Gloo Gateway to fail.
...
# Run the script to verify that the plugin(s) can be loaded by Gloo Gateway
...
#### Wrapping it up
If you followed the guide to this point, you should have an image that is guaranteed to be compatible with Gloo Gateway.

The next step is to see how to set up Gloo Gateway to use your plugins.

## Configuring Gloo Gateway to load your plugins
The Gloo Gateway `extauth` server loads plugins from a directory in the file system it has access to. It is possible to 
...
Here we will see how to accomplish the same result by editing the raw Gloo Gateway Enterprise YAML manifest.
...
{{% notice note %}}
Currently, Gloo Gateway expects to find the plugin files in the `/auth-plugins` directory. We plan to make this location configurable soon.
{{% /notice %}}
...
to deny the request will cause execution to be interrupted and Gloo Gateway to return the response generated by the step that 
denied it. No steps after the failing one will be executed.

{{% notice note %}}
Some of the external auth schemes that Gloo Gateway provides out-of-the-box take advantage of this feature. 
...
// Response returned by authorization services to the Gloo Gateway ext-auth server
...
type. Gloo Gateway conforms to the Envoy API semantics when merging headers in plugin chains. If you go to the linked Envoy docs 
...
To give a concrete example, suppose Gloo Gateway receives a request that with the following header:
...
## Conclusion
If you got to this point, we hope that you have a good understanding of how the Gloo Gateway Ext Auth plugin framework works and 
that you are ready to start hacking away! If you have any questions or ideas about how to improve this guide, please 
contact us on our [**Slack**](https://slack.solo.io) or open an issue in the [Gloo Gateway repository](https://github.com/solo-io/gloo), 
