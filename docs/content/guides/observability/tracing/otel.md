...
Gloo supports changing the Span Name from the default using the transformation filter. The following configuration can be applied on the virtual service to edit the span name:
...
Note that the `spanTransformer.name` field is an Inja template, so it is possible to use header values and any other macro logic that is supported in transformations. For more information, consult the transformation documentation.

With this change, we can see that the span name in the OpenTelemetry collector now is set to the value of the `Host` header.

If we modify the request we sent above like so:

```sh
curl -H "Host: my-hostname" http://localhost:8080
```

Then we should see the following span appear in the collector output:
