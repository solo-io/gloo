...
Gloo Gateway's service discovery leverages existing registry or catalog implementations. A widely deployed service for registering and discovery services is [HashiCorp Consul](https://www.hashicorp.com/products/consul/). If your services already register into Consul, we can use Gloo Gateway to read the service catalog from Consul and discover these services automatically.

When services registered into Consul require a bit more fine-grained grouping and routing, Gloo Gateway can do that too.

## Configuring Gloo Gateway to Discover from Consul

Gloo Gateway can automatically discover [Upstreams]({{% versioned_link_path fromRoot="/introduction/architecture/concepts#upstreams" %}}) from Consul. You can also explicitly create upstreams from Consul. 
...
If you have Discovery enabled, Gloo Gateway will automatically discover upstreams from the Consul cluster. After enabling this setting, go check your upstreams:
{{< tabs >}}
...
Even if you opt not to automatically discovery upstreams from Consul (ie, you disable Discovery), you can create them explicitly. You still need the Consul server settings in the `settings` configuration for Gloo Gateway.
...
Also, with using Upstreams instead of the consul-specific config, you can also leverage the fact that Gloo Gateway does [function discovery]({{% versioned_link_path fromRoot="/introduction/architecture/concepts/#functions" %}}) (ie, REST or gRPC based on swagger or reflection respectively). 
...
>}} settings. This allows you to target a subset of these service instances via the optional `tags` and `dataCenters` fields. Gloo Gateway will detect the correspondent IP addresses and ports and load balance traffic between them. 


If the ports and data centers for all of the endpoints for a Consul service are the same, and you don't need to slice and dice them up into finer-grained subsets, you can just use [Upstreams]({{% versioned_link_path fromRoot="/introduction/architecture/concepts/#upstreams" %}}) like you do with any other service to which to route. Also, with using Upstreams instead of the consul-specific config, you can also leverage the fact that Gloo Gateway does [function discovery]({{% versioned_link_path fromRoot="/introduction/architecture/concepts/#functions" %}}) (ie, REST or gRPC based on swagger or reflection respectively).

{{% notice note %}}
When providing the `tags` option, Gloo Gateway will only match service instances that **exactly** match the given tag set.
{{% /notice %}}
...
{{% notice note %}}
As is the case with [`Subsets`]({{% versioned_link_path fromRoot="/guides/traffic_management/destination_types/subsets/" %}}), Gloo Gateway will fall back to forwarding the request to all available service 
