...
* * **External processing (ExtProc)**: Use the Envoy external processing (ExtProc) filter to connect an external gRPC processing server to the Envoy filter chain. The external service can manipulate headers, body, and trailers of a request or response before it is forwarded to an upstream or downstream service. For more information, see [External processing]({{% versioned_link_path fromRoot="/guides/traffic_management/extproc/" %}}).
