...
Gloo Gateway is now a fully conformant Kubernetes Gateway API implementation. The existing Gloo Gateway APIs were not changed and continue to be fully supported. To deploy a gateway proxy that is based on the Kubernetes Gateway API, see the [docs](https://docs.solo.io/gateway). 
...
ggv2 - Upstream Support: enable the use of Gloo Gateway v1 Upstreams as destinations for using routes and mirror policy from the K8s Gateway API. (https://github.com/solo-io/solo-projects/issues/6129)
