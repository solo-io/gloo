...
| `credentials` | [.gloo.solo.io.AccountCredentialsSecret](../secret.proto.sk/#accountcredentialssecret) | Secrets to authenticate as a service user. Only one of `credentials`, `aws`, `azure`, `tls`, `oauth`, `apiKey`, `header`, `encryption`, or `extensions` can be set. |
