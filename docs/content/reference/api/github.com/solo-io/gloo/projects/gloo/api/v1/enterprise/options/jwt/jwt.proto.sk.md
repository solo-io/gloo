...
| `attachFailedStatusToMetadata` | `string` | Optional: If this field is not empty, the JWT failure status code and message are added to DynamicMetadata under the provided value. This is particularly useful when logging the status. For example, if the value of `attach_failed_status_to_metadata` is 'custom_auth_failure_status' then the failure status can be accessed in the access log as '%DYNAMIC_METADATA(envoy.filters.http.jwt_authn:custom_auth_failure_status)'. |
