...
| `inheritTransformation` | `bool` | Inherit transformation config from the parent route. This field has no effect on VirtualHost-level transformations. If a RouteTable or Route wants to inherit transformations from its parent Route, set this field to true, else transformations from parents are not inherited. Transformations are ordered so the child's transformation gets priority, so in the case where a child and parent's transformation matchers are the same, only the child's transformation runs because only one transformation will run per stage. Defaults to false. |
