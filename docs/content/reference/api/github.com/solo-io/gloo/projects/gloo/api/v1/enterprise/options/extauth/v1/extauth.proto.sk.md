...
| `headerName` | `string` | When receiving a request, the k8sgateway Enterprise external auth server will look for an API key in a header with this name. This field is optional; if not provided it defaults to `api-key`. |
