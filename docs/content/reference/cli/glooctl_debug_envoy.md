...
Dump envoy admin interface to a local directory (requires Gloo running on Kubernetes)

### Synopsis

Dump envoy admin interface to a local directory (requires Gloo running on Kubernetes). This is useful for debugging failures. The dump includes the envoy config dump, stats, clusters, and listeners.
